...
export class Map{
    ...
    setBaseMap(data, options){
        let _options ={};
        Object.assign(_options,__opts,options);
        let path = d3.geoPath(_options.projection);
        ...
        this.layers.append("path")
            .datum(_options.graticule.outline)
            .attr("class", "graticule outline")
            .attr("d", path);
        this.layers.append('g')
            .selectAll('path')
            .data(data.features)
            .enter()
            .append('path')
            .attr('d',path)
            .attr('fill',(d,i) => {if(d.properties.soccers){
